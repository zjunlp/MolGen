...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def pointer_generator(self, src, decode_output, decode_attn, memory, gen_logits):
        ...
        prob_proj = nn.Linear(self.config.d_model * 2, 1).cuda()
        prob = prob_proj(torch.cat([context, decode_output], -1)).sigmoid()
    ...
    def forward(
        ...
        if pointer:
            ...
            prob = self.pointer_generator(input_ids, decode_output, decode_attn, memory, gen_logits)
            lm_logits = prob
